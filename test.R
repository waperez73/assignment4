...
prepareTest<-function(directory, rows=-1){
  ...
  subject<-read.table(file.path(directory,"subject_test.txt",sep = ""),nrows=rows, header=FALSE,stringsAsFactors = FALSE,colClasses = NA)
  
  activity<-read.table(file.path(directory,"Y_test.txt",sep = ""),nrows=rows, header=FALSE,stringsAsFactors = FALSE,colClasses = NA)
